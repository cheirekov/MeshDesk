...
- position precision е между 0 и 32 bits: `0` не споделя позиция, `32` запазва
  пълната налична GPS точност, а междинните стойности споделят приблизителна зона.

UI предлага кратки privacy presets за най-практичните стойности и отделен
Advanced numeric контрол за останалите. Документираните ориентири 10–19 bits са
показани като приблизителна ширина на зоната (от около 23.3 km до 45 m).
Стойностите 20–31 имат теоретична резолюция под 45 m, но MeshDesk изрично
предупреждава, че реалната точност остава ограничена от GPS fix-а. Preview
използва същото човешко описание, без да променя записваната protobuf стойност.
