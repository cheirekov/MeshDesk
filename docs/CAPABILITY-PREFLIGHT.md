...
Remote заявката има и два transport/result статуса, които не се смесват с
capability резултата:

- `rejected` — възелът е отговорил, но PKI/admin authorization е отказана;
- `unavailable` — няма отговор, маршрутът липсва или е възникнала временна
  transport грешка.

При `rejected` Administration картата показва върнатия error code и заключва
административните команди за избрания target. Повторната metadata проверка
остава достъпна след промяна на authorized public keys. `unavailable` не се
представя като липса на права и може да бъде проверено отново.

В capability модела MeshDesk блокира изпращането само при `unsupported`. При
`unknown` показва предупреждение, но оставя утвърдената protocol операция
достъпна. Отделно, потвърден authorization `rejected` заключва remote admin
действията, докато операторът не повтори проверката. Версията на firmware не се
сравнява с ръчно поддържан списък и от нея не се отгатват възможности.
